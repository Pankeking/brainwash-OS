...
export function ExerciseCardDetails({
  ...
  const valueStep = isTimed ? 5 : 1
  const tempValueLabel = isTimed ? formatTimedValue(tempValue) : String(tempValue)
  const logPanel = (
    <section className="rounded-[1.15rem] border border-white/8 bg-[#161d26]/92 p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
      ...
      <div className="mb-2.5 grid grid-cols-2 overflow-hidden rounded-xl border border-white/8 bg-[#202834]/85 p-1">
        <button
          onClick={(event) => {
            event.stopPropagation()
            onSetType(SetType.REPS)
          }}
          className={`rounded-lg px-3 py-2 text-[9px] font-black uppercase tracking-[0.2em] transition-all ${
      ...
      <ExerciseCardValueStepper
        ...
        decrementLabel={`-${valueStep}`}
        decrementButtonProps={getHoldHandlers(-valueStep)}
        displayValue={tempValueLabel}
        incrementLabel={`+${valueStep}`}
        incrementButtonProps={getHoldHandlers(valueStep)}
      />

      <div className="mt-2.5 grid gap-2 sm:grid-cols-[minmax(0,1fr)_9rem]">
        <button
          onClick={(event) => (event.stopPropagation(), onAdd({ type: setType, value: tempValue }))}
          className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-orange-500 px-4 shadow-[0_12px_24px_rgba(249,115,22,0.22)] transition-transform active:translate-y-[1px]"
        >
        ...
        <button
          ...
          className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-red-500/25 bg-red-500/6 px-3 text-[9px] font-black uppercase tracking-[0.18em] text-red-300 transition-colors hover:bg-red-500/10"
        >
          <Trash2 size={12} />
          Remove
        </button>
  ...
  const targetsPanel = (
    <section className="rounded-[1.15rem] border border-white/8 bg-[#161d26]/92 p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
      ...
      {hasWeeklyGoal ? (
        ...
        <div className="rounded-xl border border-dashed border-white/8 px-3 py-2 text-[10px] font-bold text-slate-500">
          No weekly goal set yet
        </div>
      )}
      <div className="mt-2.5 grid gap-2 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        <div className="rounded-xl border border-white/6 bg-[#202834]/85 p-3">
          <div className="mb-2.5 flex items-center justify-between gap-3">
            <div>
              <div className="text-[8px] font-black uppercase tracking-[0.22em] text-slate-500">
                Sets this week
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/8 bg-white/6 text-slate-200 transition-colors hover:bg-white/10"
              >
                <Minus size={14} />
              </button>
              <button
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="flex h-10 items-center justify-center rounded-xl bg-orange-500 px-3 text-[9px] font-black uppercase tracking-[0.18em] text-white shadow-[0_10px_22px_rgba(249,115,22,0.2)]"
            >
              Save
            </button>
            {weeklySetGoal !== null ? (
              <button
                ...
                className="flex h-10 items-center justify-center rounded-xl border border-white/8 bg-[#202834]/85 px-3 text-[9px] font-black uppercase tracking-[0.18em] text-slate-300"
              >
                Clear
              </button>
            ) : null}
  ...
  const pages = [
    {
      id: 'log-targets',
      label: 'Log and targets',
      content: (
        <div className="grid gap-2.5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.2fr)]">
          {logPanel}
          {targetsPanel}
        </div>
      ),
